...
function InventoryScreen({ handleAskButton }) {
  ...
  useEffect(() => {
    ...
    socket.on('updateInventory', () => fetchData());
  ...
  const handleAddChange = (e) => setNewItem(prev => ({ ...prev, [e.target.name]: e.target.value }));
  const handleEditChange = (e) => setSelectedItem(prev => ({ ...prev, [e.target.name]: e.target.value }));

  const handleAddSubmit = async (item) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_CREATE_INVENTORY_ITEM}`, item);
      if (response.data.success) setShowAddModal(false);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleEditSubmit = async (item) => {
    try {
      ...
      if (response.data.success) setShowEditModal(false);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      await axios.put(`${import.meta.env.VITE_DELETE_INVENTORY_ITEM}/${id}`);
  ...
  return (
    <Container className="p-0" fluid>
      <Card className='p-3 shadow-sm'>
        <h2 className="mb-4 text-center fw-bold">Inventory Management</h2>

        <Row className="mb-3">
          <Col md={6}>
            <Form.Control
              type="text"
              placeholder="Search item name, serial number, or category..."
              value={search}
          ...
          <Col md={3} className="d-flex justify-content-end">
            <Button variant='dark' className="fs-6" onClick={() => setShowAddModal(true)}>
            ...
            {loading ? (
              ...
              currentData.map(item => (
                <tr key={item.id}>
                  ...
                  <td>
                    <OverlayTrigger placement="top" overlay={<Tooltip>Ask about this item</Tooltip>}>
                      <span
                      ...
                      >
                        <TextTruncate text={item.item_name} maxLength={18} />
                      </span>
                    </OverlayTrigger>
                  </td>
                  <td>{item.category}</td>
