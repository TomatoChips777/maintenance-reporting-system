...
//         }
//         fetchStaff();
//         navigate('/view-report', { state: { maintenance_id: report.maintenance_id, staff } });
...
function Reports() {
    ...
    const handleViewReportPage = async (report) => {
        if (report.viewed === 0) {
            try {
                await axios.put(`${import.meta.env.VITE_MARK_AS_VIEWED}/${report?.id}`);
            } catch {}
        }
        fetchStaff();
        navigate('/view-report', { state: { maintenance_id: report.maintenance_id, staff } });
    ...
    return (
        <Container fluid className="p-0">
            <Card className="p-2">
                <h3 className="text-center mb-3 fw-semibold">Reports Management</h3>

                <Row className="mb-3 px-3 align-items-end small">
                    {/* Search */}
                    <Col md={4}>
                        <Form.Group controlId="searchReports">
                    ...
                    <Col md={3}>
                        <Form.Group controlId="filterPriority">
                            <Form.Label className="small fw-semibold">Filter by Priority</Form.Label>
                            <Form.Select
                                size="sm"
                                value={priorityFilter}
                                onChange={(e) => setPriorityFilter(e.target.value)}
                    ...
                    {/* New Report Button */}
                    <Col md={2} className="d-flex justify-content-end">
                        <Button variant="dark" size="sm" onClick={() => setShowCreateModal(true)}>
                ...
                {/* Reports Table */}
                <Table striped bordered hover responsive className="mb-0 small align-middle">
                        ...
                        {loading ? (
                            ...
                            currentData.map((report, index) => (
                                <tr key={index}>
                                    <td>{FormatDate(report.created_at)}</td>
                                    <td>{report.reporter_name}</td>
                                    <td><TextTruncate text={report.location} maxLength={25} /></td>
                                    <td>
                                        <TextTruncate
                                            text={
                                                report?.description
            ...
            {/* Modals */}
            <ViewReport show={showViewModal} handleClose={() => setShowViewModal(false)} report={selectedReport} staff={staff} />
            <CreateReport show={showCreateModal} handleClose={() => setShowCreateModal(false)} staff={staff} />
            <ArchiveAlert show={showAlert} handleClose={() => setShowAlert(false)} report={reportToRemove} />
        </Container>
    );
}
