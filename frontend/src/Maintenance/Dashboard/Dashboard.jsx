...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_DASHBOARD_DATA}/${user.id}`);
      setReports(res.data.reportFrequencyResult || []);
      setQuickStats(res.data.quickStats || []);
      setReportsToday(res.data.reportsTodayList || []);
      setInProgressReports(res.data.inProgressList || []);
  ...
  if (loading) {
    return <div className="text-center mt-5"><Spinner animation="border" size="sm" variant="primary" /></div>;
  }
    ...
    <div>
      ...
      <Card className="mb-4">
        <Card.Body>
          <Row className="g-3">
            {[
              { label: "In Progress", value: quickStats.inProgessCount },
              { label: "Urgent Task", value: quickStats.urgentReports },
              { label: "High Priority", value: quickStats.highPriorityReports },
              { label: "Medium Priority", value: quickStats.mediumPriorityReports },
              ...
              <Col key={index} xs={12} sm={6} md={2} className="flex-grow-1">
                <Card text="white" className="h-100 shadow-sm bg-quick-stats">
                  <Card.Body className="text-center p-2">
                    ...
                    <Card.Text style={{ fontSize: "0.85rem" }}>{label}</Card.Text>
                  </Card.Body>
                </Card>
              </Col>
            ))}
      ...
      <Row className="mb-4">
        ...
        <Col md={6}>
          <Card className="mb-3">
            <Card.Header className="fw-semibold small">Reports Frequency</Card.Header>
            <Card.Body>
              <Charts type="reportsFrequency" data={reports} />
            </Card.Body>
          </Card>
          ...
          <Card className="mb-3">
            <Card.Header className="fw-semibold small">Reports Summary</Card.Header>
            <Card.Body>
              <Charts type="reportStatus" data={categoryData} />
            </Card.Body>
          </Card>
          ...
          <Card className="mb-3">
            <Card.Header className="fw-semibold small">Assigned Frequency</Card.Header>
            <Card.Body>
              <Charts type="assignedStaffFrequency" data={assignedStaffFrequency} />
            </Card.Body>
          </Card>
          ...
          <Card className="mb-3">
            <Card.Header className="fw-semibold small">Borrowers Frequency</Card.Header>
            <Card.Body>
              <Charts type="borrowerRanking" data={null} />
            </Card.Body>
          </Card>
        ...
        <Col md={6}>
          {/* Reports Today */}
          <Card className="mb-4">
            <Card.Header className="fw-semibold small">Recent Reports</Card.Header>
            <Card.Body className="small">
              {reportsToday.length === 0 ? (
                <p className="mb-0 text-muted small">No task reported today</p>
              ) : (
                <Accordion flush>
                  {reportsToday.map((report, idx) => (
                    <Accordion.Item eventKey={String(idx)} key={idx}>
                      <Accordion.Header className="small">
                        <div className="w-100 d-flex justify-content-between">
                          <span>{report.location}</span>
                          <span className="text-muted">{FormatDate(report.created_at, 'short')} | {report.time}</span>
                        </div>
                      </Accordion.Header>
          ...
          {/* In Progress */}
          <Card className="mb-4">
            <Card.Header className="fw-semibold small">In Progress Reports</Card.Header>
            <Card.Body className="small">
              {inProgressReports.length === 0 ? (
                <p className="mb-0 text-muted small">No reports in progress</p>
              ) : (
                <Accordion flush>
                  {inProgressReports.map((report, idx) => (
                    <Accordion.Item eventKey={String(idx)} key={report.id}>
                      <Accordion.Header className="small">
                        <div className="w-100 d-flex justify-content-between">
                          <span>{report.location}</span>
                          <span className="text-muted">{FormatDate(report.updated_at, 'short')} | {report.time}</span>
                        </div>
                      </Accordion.Header>
                      <Accordion.Body className="small">
                        <strong>Priority:</strong> {report.priority} <br />
                        ...
                        {report.assigned_staff_names && (
                          <>
                            <small className="text-muted fw-bold">Assigned Staff</small>
                            <ul className="mb-0">
                              <li style={{ fontSize: "0.8rem" }}>{report.assigned_staff_names}</li>
                            </ul>
                          </>
                        )}
                      </Accordion.Body>
          ...
          {/* Completed */}
          <Card className="mb-4">
            <Card.Header className="fw-semibold small">Recently Completed</Card.Header>
            <Card.Body className="small">
              {recentlyCompletedReports.length === 0 ? (
                <p className="mb-0 text-muted small">No reports completed recently</p>
              ) : (
                <Accordion flush>
                  {recentlyCompletedReports.map((report, idx) => (
                    <Accordion.Item eventKey={String(idx)} key={report.id}>
