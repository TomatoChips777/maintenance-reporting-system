...
function Staff() {
  ...
  const handleEditSubmit = async (staff) => {
    if (!staff || !staff.id) throw new Error("Staff ID missing.");
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_UPDATE_MAINTENANCE_STAFF_DETAILS}/${staff.id}`,
        staff
  ...
  return (
    <Container fluid className="p-0">
      <Card className="p-2">
        <h4 className="mb-3 text-center fw-bold" style={{ fontSize: "1.25rem" }}>
          Staff Management
        </h4>
        ...
        <Table striped bordered hover responsive className="mb-0 small">
          ...
          <tbody style={{ fontSize: "0.85rem" }}>
            {loading ? (
              ...
              <tr>
                <td colSpan="7" className="text-center">
                  <Alert variant="light" className="mb-0 small">No records found.</Alert>
                </td>
              </tr>
            ) : (
              currentData.map(staff => (
